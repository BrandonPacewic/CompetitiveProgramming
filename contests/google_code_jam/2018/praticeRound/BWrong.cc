...
void printPairs(T_Pairs pairs, bool pairSpacing = true,
                bool newPairSpace = true, bool newLine = true) {
    for (int i = 0; i < pairs.size(); i++) {
        ...
        if (pairs[i].second != ' ') cout << (newPairSpace ? ' ' : '\n');
    }
    ...
    if (N == 2) {
        if (p[0] != p[1]) {
            ...
            ans.push_back(pair<char, char>(
                remove.second, remove.first == 2 ? remove.second : ' '));
        }
        ...
        char pos_char = currentMax.second == 0   ? 'A'
                        : currentMax.second == 1 ? 'B'
                                                 : 'C';
        int nextHighest = currentMax.second == 0   ? max(p[1], p[2])
                          : currentMax.second == 1 ? max(p[0], p[2])
                                                   : max(p[0], p[1]);

        if (currentMax.first - nextHighest != 0)
            ans.push_back(pair<char, char>(
                pos_char,
                currentMax.first - nextHighest == 2 ? pos_char : ' '));

        int index_min = 0;
        for (int i = 0; i < N; i++) {
            if (p[index_min] > p[i]) index_min = i;
        }

        pos_char = index_min == 0 ? 'A' : index_min == 1 ? 'B' : 'C';
        for (int i = 0; i < p[index_min];) {
            ans.push_back(pair<char, char>(
                pos_char, p[index_min] - 2 >= 0 ? pos_char : ' '));
            i += p[index_min] - 2 >= 0 ? 2 : 1;
        }

        pair<char, char> reduce(index_min == 0 ? 'B' : 'A',
                                index_min < 2 ? 'C' : 'B');
        int finalVal = index_min == 0   ? min(p[1], p[2])
                       : index_min == 1 ? min(p[0], p[2])
                                        : min(p[0], p[1]);
        for (int i = 0; i < finalVal; i++) ans.push_back(reduce);
    }
