...
template <typename T_List, typename Type>
bool findInList(T_List List, Type find) {
    for (Type i : List)
...
void runCase(int tc) {
    auto trace = [](vector<vector<int>> matrix) -> int {
        int total = 0;
        for (int i = 0; i < matrix.size(); i++) total += matrix[i][i];
    ...
    cout << "Case #" << tc << ": " << trace(matrix) << ' ' << rows << ' '
         << columns << '\n';
}
