...
const void output_empty_matrix(
    const uint16_t& rows, const uint16_t& cols, const char& primary,
    const char& secondary,
    std::function<bool(uint16_t&, uint16_t&)> evaluation,
...
template <typename ForwardIterator, typename ValueType>
void matrix_iota(ForwardIterator first, ForwardIterator last,
                 ValueType value) {
   for (; first != last; ++first) {
...
template <typename ForwardIterator, typename ValueType,
          typename ReturnContainerType = std::vector<ValueType>>
[[nodiscard]] ReturnContainerType create_linear_sequence(
...
[[nodiscard]] const Type matrix_trace(ForwardIterator first,
                                      ForwardIterator last,
                                      const Type& value = 0) {
   Type trace = value;
