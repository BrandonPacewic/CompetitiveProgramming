...
#define CPL_IS_CONTAINER(T)       \
   static_assert(is_container<T>, \
                 "Templated parameter is not a valid container.")
...
OutputContainer alternating_insertion(const InputContainer1& input1,
                                      const InputContainer2& input2,
...
#endif  // CPL
   return alternating_insertion(input1.begin(), input1.end(), input2.begin(),
                                input2.end(), output.begin());
}
...
template <typename T_container,
          typename ContainerValueType = typename T_container::value_type>
[[nodiscard]] std::vector<std::pair<ContainerValueType, uint16_t>>
run_length_encoding(const T_container& container) {
#if CPL
...
template <typename T_container,
          typename ContainerValueType = typename T_container::value_type>
[[nodiscard]] std::set<ContainerValueType> to_set(
...
template <typename T_container,
          typename ContainerValueType = typename T_container::value_type>
[[nodiscard]] std::unordered_set<ContainerValueType> to_unordered_set(
    const T_container& container) {
#if CPL
   CPL_IS_CONTAINER(T_container);
#endif  // CPL
