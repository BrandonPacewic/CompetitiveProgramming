...
template <typename T>
struct is_container_internal<T, std::conditional_t<false,
...
OutputContainer alternating_insertion(
    const InputContainer1& input1, const InputContainer2& input2, OutputContainer output) {
#if CPL
...
[[nodiscard]] std::vector<std::pair<BaseIteratorType, uint16_t>> run_length_encoding(
